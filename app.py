...
llm = AzureChatOpenAI(api_key=os.environ["AZURE_OPENAI_API_KEY"], api_version="2023-12-01-preview",
                      azure_endpoint=os.environ["AZURE_OPENAI_ENDPOINT"], model=os.environ["AZURE_OPENAI_DEPLOYMENT_NAME"], streaming=True)
...
def img_prompt_func(data_dict):
    ...
    if data_dict["context"]["images"]:
        for image in data_dict["context"]["images"]:
            image_message = {"type": "image_url", "image_url": {"url": f"{image}"}, }
...
vector_store_multi_modal: AzureSearch = AzureSearch(azure_search_endpoint=os.environ["AZURE_SEARCH_ENDPOINT"],
                                                    ...
                                                    index_name=index_name, embedding_function=embedding_function, )
...
async def start():
    ...
    message_history = ChatMessageHistory(session_id=identifier + str(uuid4()), )
    chain_multimodal_rag_with_source = RunnableMap(
        {"documents": retriever_multi_modal, "question": RunnablePassthrough()}) | {
                                           "source_documents": lambda input: [doc.metadata for doc in
                                                                              input["documents"]],
                                           "answer": RunnableLambda(
                                               lambda input: {"context": get_image_text(input["documents"]),
                                                              "question": input["question"]}) | RunnableLambda(
                                               img_prompt_func) | llm | StrOutputParser()}
...
async def main(message: cl.Message):
    msg = cl.Message(content="what is databricks")
    ...
    if source_documents:
        for source_idx, source_doc in enumerate(source_documents):
            ...
            if source_doc.get("image"):
                ...
                elements.append(cl.Text(content=source_doc.get("header"), name=source_name))

...
from azure.search.documents.indexes.models import (SearchableField, SearchField, SearchFieldDataType,
                                                   SimpleField)
...
llm = AzureChatOpenAI(api_key=os.environ["AZURE_OPENAI_API_KEY"], api_version="2024-03-01-preview",
                      azure_endpoint=os.environ["AZURE_OPENAI_ENDPOINT"], model=os.environ["AZURE_OPENAI_DEPLOYMENT_NAME"], streaming=False,
                      max_tokens=1024)

aoai_embeddings = AzureOpenAIEmbeddings(api_key=os.getenv("AZURE_OPENAI_API_KEY"),
    azure_deployment= os.environ["AZURE_OPENAI_ADA_EMBEDDING_DEPLOYMENT_NAME"], openai_api_version="2024-03-01-preview",
    azure_endpoint=os.environ["AZURE_OPENAI_ENDPOINT"])

embedding_function = aoai_embeddings.embed_query
fields = [SimpleField(name="id", type=SearchFieldDataType.String, key=True, filterable=True, ),
    ...
    SearchField(name="content_vector", type=SearchFieldDataType.Collection(SearchFieldDataType.Single), searchable=True,
        vector_search_dimensions=len(embedding_function("Text")), vector_search_profile_name="myHnswProfile", ),
    SearchableField(name="metadata", type=SearchFieldDataType.String, searchable=False, ),
    # Additional field to store the title
    SearchableField(name="header", type=SearchFieldDataType.String, searchable=True, ),
    # Additional field for filtering on document source
    SimpleField(name="image", type=SearchFieldDataType.String, filterable=False, searchable=False, ), ]

index_name: str = "langchain-vector-demo-custom2"

vector_store_multi_modal: AzureSearch = AzureSearch(azure_search_endpoint=os.environ["AZURE_SEARCH_ENDPOINT"],
    azure_search_key=os.environ["AZURE_SEARCH_KEY"], index_name=index_name, embedding_function=embedding_function,
    fields=fields, )
...
def process(pdf_path):
    loader = AzureAIDocumentIntelligenceLoader(file_path=pdf_path, api_key=os.getenv("AZURE_DOCUMENT_INTELLIGENCE_KEY"),
    ...
    for doc in child_docs:
        ...
        if figure_indices:
            for figure_indice in figure_indices:
                ...
                doc_result = Document(page_content=doc.page_content,
                                      metadata={"header": json.dumps(doc.metadata), "source": "sam.pdf",
                                                "image": image})
                lst_docs.append(doc_result)
        else:
            doc_result = Document(page_content=doc.page_content,
                                  metadata={"header": json.dumps(doc.metadata), "source": "sam.pdf", "image": None})
            lst_docs.append(doc_result)
    vector_store_multi_modal.add_documents(documents=lst_docs)


# process("D:/MS/Learning/Databrick/Databricks Delta eBook_Final.pdf")
